...
\section{研究目的與方法}
 本研究分兩大部分，第一運用Open AI Gym的Pong-v0來訓練機器學習，第二利用CoppilaSim模擬環境並訓練出能與人對打的機電系統。\\[12pt]
...
\fi %整段註解(結尾)
\subsection{優化器}
 為了讓機器學習的錯誤率降低，因此利用優化器來降低loss function的值，在error surface上找到最小值，即是找到錯誤率最低的地方。也可以改善啟動函數梯度消失的問題，以下將介紹幾種優化的方法：
\begin{itemize}
...
\qquad 強化學習與其他學習法不一樣的地方在於：不需要事先收集大量數據提供當作學習樣本，而是透過與環境互動，在環境下發生的狀態當作學習的來源。
