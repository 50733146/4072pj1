...
#resume = True # resume training from previous checkpoint (from pong1.2_save.p  file)?
reward_sum = 0
episode_number = 0
...
if resume:
  ...
  # resume reward history
  reward_list = np.loadtxt("pong1.2_reward_list.txt")
  sum_reward_list = np.loadtxt("pong1.2_sum_reward_list.txt")
...
while True:
  ...
  if done: # an episode finished
    ...
    if episode_number % 100 == 0: pickle.dump(model, open('pong1.2_save.p', 'wb'))
    if episode_number % 100 == 0: np.savetxt("pong1.2_reward_list.txt", reward_list, fmt="%s")
    if episode_number % 100 == 0: np.savetxt("pong1.2_sum_reward_list.txt", sum_reward_list, fmt="%s")
    reward_sum = 0
