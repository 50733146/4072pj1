...
pickle_name = "pong1.1_save.ac"
reward_list = np.array(())
reward_list_name = "pong1.1_rlist.txt"
sum_reward_list = np.array(())
sum_reward_list_name = "pong1.1_Srlist.txt"

# model initialization
D = 80 * 80 # input dimensionality: 80x80 grid
if resume:
  model, model_target = pickle.load(open(pickle_name, 'rb'))
else:
  model = {}
...
while True:
  ...
  if reward != 0:
    ...
    print (('round %d game finished, reward: %d' % (times, reward)) + ('' if reward == -1 else ' !!!!!!!!'))
  ...
  if done:
    ...
    print ('ep %d reward total was %d. running mean: %f' % (times, reward_sum, running_reward))
    reward_sum = 0
